...
async def start(client, message):
    # Common buttons used in both cases
    buttons = [
        [
    ...
    # If message is from a group/supergroup
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        if not await db.get_chat(message.chat.id):
            ...
            await client.send_message(
                LOG_CHANNEL,
                script.LOG_TEXT_G.format(message.chat.title, message.chat.id, total_members, "Unknown")
            )
            await db.add_chat(message.chat.id, message.chat.title)
        return  # Stop execution after group handling
    ...
    if not await db.is_user_exist(user_id):
        ...
        await client.send_message(
            LOG_CHANNEL,
            script.LOG_TEXT_P.format(user_id, user_mention)
        )

    # Command validation (must be exactly 2 arguments)
    if len(message.command) != 2:
        await message.reply_photo(
            photo=random.choice(PICS),
            caption=script.START_TXT.format(user_mention, temp.U_NAME, temp.B_NAME),
            reply_markup=reply_markup,
    ...
    if not files_:
        ...
        decoded_str = decoded_bytes.decode("utf-8")  # safer than 'ascii'
        ...
        try:
            ...
            title = '@KR_PICTURE ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@KR_Picture {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
